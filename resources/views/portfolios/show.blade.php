...
<section class="pt-32 pb-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="aspect-video rounded-2xl overflow-hidden border border-custom mb-10">
            <img src="/storage/portfolio/{{ $portfolio->image ? str_replace('storage/portfolio/', '', $portfolio->image) : '' }}" alt="{{ $portfolio->title }}" class="w-full h-full object-contain bg-white">
        </div>
        <span class="text-accent text-sm font-mono">{{ $portfolio->category }}</span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            ...
            <div class="card p-4">
                ...
                <p class="text-sm font-semibold mt-1">{{ $portfolio->category }}</p>
            </div>
            @endif
            @if($portfolio->url)
            <div class="card p-4">
                <p class="text-xs text-gray-500">Live URL</p>
                <a href="{{ $portfolio->url }}" target="_blank" class="text-sm font-semibold mt-1 text-accent hover:underline inline-block">Visit Site →</a>
            </div>
            @endif
        </div>
